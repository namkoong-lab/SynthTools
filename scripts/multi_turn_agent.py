...
Usage:
  python scaling_tools/scripts/multi_turn_agent.py \
    scaling_tools/tool_content/tool_final/task_curated/<task>.yml \
    --metadata_folder scaling_tools/tool_content/tool_final/tool_meta \
    --log_folder scaling_tools/tool_content/tool_final/agent_trajectories \
    --max_interactions 20

  python scaling_tools/scripts/multi_turn_agent.py \
    scaling_tools/tool_content/tool_final/task_curated/<task1>.yml \
    scaling_tools/tool_content/tool_final/task_curated/<task2>.yml \
    scaling_tools/tool_content/tool_final/task_curated/<task3>.yml \
    --metadata_folder scaling_tools/tool_content/tool_final/tool_meta \
    --log_folder scaling_tools/tool_content/tool_final/agent_trajectories \
    --max_interactions 20
"""
...
def get_or_create_client(selected_tool_name: str, client_cache: dict, tools_registry: dict, filtered_metadata_file: Path, metadata_folder: Path = None, spec_id: str = None):
    ...
    metadata_template_path = script_dir / "prompt_templates" / "tool_simulator_template_metadata.yml"
...
def load_agent_system_prompt() -> str:
    """Load the agent system prompt from multi_turn_template.yml."""
    script_dir = Path(__file__).resolve().parent.parent
    template_path = script_dir / "prompt_templates" / "multi_turn_template.yml"
...
def main():
    ...
    parser.add_argument("task_files", nargs="+", type=str, help="Path(s) to task YAML file(s) containing meta_data and tools_data")
    ...
    for task_idx, task_file in enumerate(task_files):
        ...
        try:
            ...
            try:
                task_and_tools_header = build_multi_turn_user_prompt(task_yaml)
            ...
            while interaction_count < args.max_interactions and not task_completed:
                interaction_count += 1
            ...
            if not task_completed:
                print(f"Reached max interactions ({args.max_interactions}) for task {task_file.name}. Moving to next task.")
